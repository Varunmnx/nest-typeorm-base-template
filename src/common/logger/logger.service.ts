import { MicroServiceEnvKeys, MicroserviceEnvVariables } from '@/microserviceFactory.factory';
...
export class LoggerService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.winstonLogger = winston.createLogger({
      ...
      format: winston.format.combine(winston.format.timestamp()), // Define log format
      transports: [
        ...
        new winston.transports.Console({
          format: winston.format.combine(
            ...
            winston.format.printf(({ timestamp, level, message, context, trace }) => {
              const serviceName = this.configService.getOrThrow('SERVICE_NAME');
              let formattedMessage = `[${serviceName}] ${timestamp} \t [${level.toUpperCase()}]${
                context ? ` [${context}]` : ''
              }: `;
